...
from uit_plus_job.workflows.uit_job_RWS import SpatialUitJobRWS
from uit_plus_job.services.workflow_manager.uit_workflow_manager import ResourceWorkflowUitJobManager
...
class SpatialUitJobMWV(MapWorkflowView):
    ...
    def render_uit_job_table(self, request, resource, workflow, current_step, previous_step, next_step):
        """
        Render uit jobs table showing the status of the current job that is processing.
            request(HttpRequest): The request.
            session(sqlalchemy.Session): the session.
            resource(Resource): the resource for this request.
            workflow(ResourceWorkflow): The current workflow.
            current_step(ResourceWorkflowStep): The current step to be rendered.
        Returns:
            HttpResponse: The uit job table view.
        """
        job_id = current_step.get_attribute('job_id')
        app = self.get_app()
    ...
    def run_job(self, request, session, resource, workflow_id, step_id, *args, **kwargs):
        """
        Handle run-job-form requests: prepare and submit the uit job.
        """
        ...
        if not jobs:
            raise RuntimeError('Improperly configured SpatialUitJobRWS: no "jobs" option supplied.')
        ...
        # Get Job Script
        job_script = step.options.get('job_script', None)
        ...
        # Setup the uit Workflow
        uit_job_manager = ResourceWorkflowUitJobManager(
            session=session,
            model_db=model_db,
        ...
        params_file_path = os.path.join(uit_job_manager.workspace, 'workflow_params.json')
        with open(params_file_path, 'w') as params_file:
            params_file.write(serialized_params)

        # Add parameter file to workflow input files
        uit_job_manager.input_files.append(params_file_path)

        # Prepare the job
        job_id = uit_job_manager.prepare()

        # Submit job
        uit_job_manager.run_job()
        ...
        step.set_attribute('uit_job_id', job_id)
